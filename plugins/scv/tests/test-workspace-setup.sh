...
mode()  { WS_CODEX="$1/scv/CODEX.md" WS_MANIFEST="$1/scv/WORKSPACE.yaml" bash -c 'source "'"$LIB"'"; scv_resolve_mode'; }
field() { WS_CODEX="$1/scv/CODEX.md" bash -c 'source "'"$LIB"'"; '"$2"; }
...
[[ -f "$P1/scv/CODEX.md" ]] && ok "scv/CODEX.md created" || fail "scv/CODEX.md missing"
...
replace_marker_block "$P1/scv/CODEX.md" "PROJECT:LOCAL START" "PROJECT:LOCAL END" "MY-LOCAL-RULE-XYZ"
...
grep -q "MY-LOCAL-RULE-XYZ" "$P1/scv/CODEX.md" && ok "PROJECT:LOCAL survives sync" || fail "PROJECT:LOCAL lost on sync"

# 5. DETACH — clear root → SINGLE again, no migration
replace_marker_block "$P1/scv/CODEX.md" "SCV:WORKSPACE START" "SCV:WORKSPACE END" \
  '```yaml
repo_id: be
role: backend
root:
