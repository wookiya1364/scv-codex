...
PROTOCOL_ROOT="$STANDARD_ROOT/references/protocols"
...
for root_instruction in AGENTS.md CODEX.md CLAUDE.md; do
...
# scv/ hierarchy (now includes CODEX.md)
for f in CODEX.md INTAKE.md PROMOTE.md RALPH_PROMPT.md ARCHITECTURE.md DESIGN.md DOMAIN.md AGENTS.md TESTING.md REPORTING.md; do
...
assert_contains "$APP/scv/CODEX.md" "<!-- STANDARD:VERSION -->${VERSION_NOW}<!-- /STANDARD:VERSION -->"
assert_contains "$APP/scv/CODEX.md" "<!-- STANDARD:SYNCED_AT -->$(date +%Y-%m-%d)<!-- /STANDARD:SYNCED_AT -->"
...
echo
echo '=== [11g] $scv:work workflow + helper ==='
WORK_SH="$STANDARD_ROOT/scripts/work.sh"
WORK_CMD="$PROTOCOL_ROOT/work.md"
assert_file "$WORK_SH"
...
echo
echo '=== [11g'\''] $scv:codegen workflow (TDD-first variant of $scv:work) ==='
...
assert_out_contains "scv-codex"     "$OUT" "update.sh: extracts marketplace name 'scv-codex'"

# command protocol content
assert_contains "$UPDATE_CMD" "codex plugin marketplace upgrade"
assert_contains "$UPDATE_CMD" "codex plugin add"
...
echo
echo '=== [11e] $scv:promote workflow protocol ==='
...
assert_out_contains '$scv:install-deps'  "$HELP_DEP_MISSING_OUT"  'help.sh: install hint references $scv:install-deps'
...
assert_out_contains "graphify (Codex skill" "$CHECK_OUT" "install-deps --check: graphify section header"
...
assert_out_contains "Codex skill" "$HELP_GRAPHIFY_OUT" "help.sh: graphify is identified as a Codex skill"
assert_out_contains '$scv:install-deps' "$HELP_GRAPHIFY_OUT" 'help.sh: install hint references the Codex skill'
...
# --- scv/CODEX.md 의 adoption 강화 단락 ---
SCV_CODEX="$STANDARD_ROOT/template/scv/CODEX.md"
assert_contains "$SCV_CODEX" "N/A is a steady state, not a backlog"
assert_contains "$SCV_CODEX" "Lift one doc at a time when there's a real driver"
...
# --- Repository documentation: Codex entry point, loop, refs, and locale parity ---
REPO_README="$REPO_ROOT/README.md"
REPO_README_KO="$REPO_ROOT/README.ko.md"
REPO_README_JA="$REPO_ROOT/README.ja.md"
assert_file "$REPO_README"
assert_file "$REPO_README_KO"
assert_file "$REPO_README_JA"

assert_contains "$REPO_README" "# SCV for Codex"
assert_contains "$REPO_README" "## Quick start"
assert_contains "$REPO_README" "## Five-minute walkthrough"
assert_contains "$REPO_README" "## The loop"
assert_contains "$REPO_README" "## Why SCV?"
assert_contains "$REPO_README" '`PLAN.md` is the source of truth'
assert_contains "$REPO_README" 'external tickets stay linked through `refs:`'
assert_contains "$REPO_README" 'literal `$scv:<name>` form'

assert_contains "$REPO_README_KO" "## 빠른 시작"
assert_contains "$REPO_README_KO" "## 5분 워크스루"
assert_contains "$REPO_README_KO" "## 흐름"
assert_contains "$REPO_README_KO" "## 왜 SCV인가?"

assert_contains "$REPO_README_JA" "## クイックスタート"
assert_contains "$REPO_README_JA" "## 5 分ウォークスルー"
assert_contains "$REPO_README_JA" "## ループ"
assert_contains "$REPO_README_JA" "## なぜ SCV か"
...
# promote protocol — Step 6 새 도식 단계 + Codex-native questions
...
# just documentation — if Codex ever executes the fence without substituting the
...
assert_contains "$HANDOFF_CMD" '"${SCV_PLUGIN_ROOT}/scripts/handoff.sh" "<module>" write --to "<to_repo>" --slug "<slug>" --title "<title>"'
...
echo
echo "=== [11eee] Codex repository and plugin README contract ==="

PLUGIN_README="$STANDARD_ROOT/README.md"
PLUGIN_README_KO="$STANDARD_ROOT/README.ko.md"
PLUGIN_README_JA="$STANDARD_ROOT/README.ja.md"
for FILE in "$PLUGIN_README" "$PLUGIN_README_KO" "$PLUGIN_README_JA"; do
  assert_file "$FILE"
  assert_contains "$FILE" "# SCV for Codex"
  assert_contains "$FILE" "codex plugin marketplace add"
  assert_contains "$FILE" "codex plugin add scv@scv-codex"
  assert_contains "$FILE" '$scv:help'
  assert_contains "$FILE" '$scv:set-models'
  assert_contains "$FILE" "codex plugin marketplace upgrade scv-codex"
done

assert_contains "$PLUGIN_README" "SCV uses explicit Codex skills, not slash commands"
assert_contains "$PLUGIN_README" "Codex plugin skills cannot pin a"
assert_contains "$PLUGIN_README" "read-only migration"
assert_contains "$PLUGIN_README" "Then start a new Codex session"
assert_contains "$PLUGIN_README_KO" "## 설치"
assert_contains "$PLUGIN_README_KO" "## Codex 호환성"
assert_contains "$PLUGIN_README_JA" "## インストール"
assert_contains "$PLUGIN_README_JA" "## Codex 互換性"

mermaid_count=$(cat "$REPO_README" "$REPO_README_KO" "$REPO_README_JA" 2>/dev/null | grep -c '^```mermaid')
if [[ "$mermaid_count" -eq 3 ]]; then
  pass "[11eee] repository README family has one workflow diagram per language"
else
  fail "[11eee] repository README family expected 3 mermaid blocks, got $mermaid_count"
fi
...
# Repository README — idea-first examples in all supported languages
assert_contains "$REPO_README" '$scv:help "I want to add a refund button"'
assert_contains "$REPO_README_KO" '$scv:help "환불 버튼을 추가하고 싶어"'
assert_contains "$REPO_README_JA" '$scv:help "払い戻しボタンを追加したい"'
...
# Repository README — archive-search examples in all supported languages
assert_contains "$REPO_README" '$scv:help "How did we handle refunds last quarter?"'
assert_contains "$REPO_README_KO" '$scv:help "지난 분기 환불 처리는 어떻게 했지?"'
assert_contains "$REPO_README_JA" '$scv:help "前四半期の払い戻し対応はどうした?"'

# plugin.json — Codex manifest and semver are validated in test-codex-plugin.sh
assert_contains "$STANDARD_ROOT/.codex-plugin/plugin.json" '"name": "scv"'

# Codex runtime adapter passes free-form arguments as a shell array.
assert_contains "$HELP_CMD" "SCV_ARGS"
assert_contains "$HELP_CMD" 'do not rebuild it with `eval`'
...
# Repository README — activation boundary is a new Codex session.
assert_contains "$REPO_README" "Start a new Codex chat or CLI session"
assert_contains "$REPO_README_KO" "새 Codex 채팅 또는 CLI 세션"
assert_contains "$REPO_README_JA" "新しい Codex chat または CLI"
