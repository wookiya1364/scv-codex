...
> 이 파일은 `~/.codex/ralph-template.md` 를 대체하도록 설계되었습니다.
> `cp ralph-template-scv.md ~/.codex/ralph-template.md` 로 교체하세요.
...
- 수정·검증마다 **`$scv:report` Codex 스킬을 호출**하여 협업툴에 진행 상황을 보고한다 (스크린샷/비디오 포함).
...
2. **표준 문서 전부 읽기 (필수 + 조건부)**: `scv/CODEX.md` 의 인덱스 순서대로 각 문서의 **현재 내용** 을 읽는다:
...
5. **Phase 완료·실패 알림**: **반드시 `$scv:report` Codex 스킬**로만 보낸다. 직접 API 호출 금지.
...
**Codex 는 Slack/Discord API 를 직접 호출하지 않는다.** 항상 `$scv:report` 스킬 경유:
...
```
$ralph-loop "~/.codex/ralph-template.md 를 읽고 실행 흐름을 따르라. 프로젝트 설정은 RALPH_PROMPT.md, 표준 명세는 scv/*.md, 보고는 $scv:report Codex 스킬을 사용한다." --max-iterations 35 --completion-promise "DONE"
```
