...
has "$ROUTINE_CMD" "report.sh" "protocol: (c) $scv:report summary command"
...
has "$ROUTINE_CMD" 'Use the arguments attached to the current action invocation as data. Preserve them as separately quoted `SCV_ARGS` elements and pass them without `eval`; never rebuild an argument string.' "protocol: carries host argument context"
