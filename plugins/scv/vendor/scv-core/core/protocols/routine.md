...
When a report is due, use the `$scv:report` format:
...
- Codex's recurring-prompt / loop feature, re-running `action:routine <name>` on the suggested cadence;
