...
Read from `$scv:help` at "→ Mode B' (archive search)". Follow it, then return to the protocol where you left off.

#### → Mode B' (archive search)

Skip Steps B0–B6 entirely. Instead:

1. Run `bash "${SCV_CORE_ROOT}/scripts/help.sh" --archive-index` and read its `ARCHIVE_INDEX:` block (`<folder> | <title> | <created_at>` per entry). If it shows `(empty)` or `(no archive yet)`, tell the user honestly and suggest `$scv:promote` to create the first plan. Stop.
