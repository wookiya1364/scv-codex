...
You — Codex — drive a promote plan to completion: **read PLAN.md + TESTS.md → implement → run tests → archive on success**. Full protocol in `scv/PROMOTE.md`.
...
If `scv/scv_settings.json` `SCV_LANG` is unset, you may suggest the user run `$scv:help` once to lock the preference (don't block the current task on it — fall back to auto-detect / English for now).
<!-- /SCV:GUIDANCE -->
...
> (`gh` / `glab` / `ffmpeg` / etc.), suggest running
...
The helper already ran `scripts/graph.sh ensure` (v0.51.0+): the SCV graph — docs links,
archived plans → files, decision refs, co-change pairs — is rebuilt automatically when
stale (bash + jq, about a second). Nothing to ask.
...
or hooks Graft; `$scv:install-deps --print` shows the optional one-liner. When the header also
...
Also surface any **external refs** from the helper's `=== external refs ===` block (grouped by `type`, e.g. `[jira] 2`, `[pr] 1`). One line per type is enough — the user can follow links without Codex reading them.

**Document split judgment** (applied from now on, through implementation):

| Signal | Codex's action |
|---|---|
...
| Neither (default) | Codex judges. If `Approach Overview` > ~50 lines, `Suggested path` (or legacy `Steps`) > ~15, or implementation reveals a dense sub-topic (ARCH / REQUIREMENTS / API / MIGRATION / tests) — **propose** split by asking the user. User accepts or declines. |
...
- Read individual entries only when:
  ...
  2. Codex judges the content of the current step needs that context (e.g., Step says "per API.md contract" — then Read API.md).
...
Relation to Ralph Loop: this paragraph is what owns `$scv:work`'s long-run behavior — no external loop harness is required for a plan to run to completion (an external loop remains an optional accelerant, not a dependency).
<!-- /SCV:GUIDANCE -->
...
append **one** entry to `scv/DECISIONS.md` with the script below; the log's rules
(append-only, seeding, the shared entry format, the index) are in
`core/contracts/decisions.md`. In a nested module, append to `<SCV_DIR>/DECISIONS.md`
and write `refs: <SCV_DIR>/archive/<slug>/PLAN.md`.
...
**author is mandatory — never write an anonymous entry** (resolve it the same
way as `$scv:promote`'s AUTHOR: `git config user.name` → `GIT_AUTHOR_NAME` → `USER`):
...
`$scv:promote`'s: PLAN.md already holds the route you were *supposed* to take,
...
[1] "Yes — move to scv/conversations/archive/"
    description: "The conversation file is moved to scv/conversations/archive/<filename>. $scv:help's 'unfinished' list won't show it anymore. Both directories are committed (v0.22.0+), so the move is visible to the whole team on the next commit."
...
When `$scv:work` is creating its first PR and the settings do not have `SCV_ATTACHMENTS_RETENTION_DAYS`, ask the user one concise question (and never again):
...
[1] "Yes — auto-create PR (recommended)"
    ...
    - Read `lang:` from the archived PLAN.md frontmatter (set by $scv:promote
      Step 0). All section headings, fixed phrases, and Codex-written narrative
...
**On [1] Yes**: Codex directly creates the folder + PLAN.md + TESTS.md scaffold via `Write`. Auto-include the epic's archived feature slugs in the PLAN.md Summary section.
