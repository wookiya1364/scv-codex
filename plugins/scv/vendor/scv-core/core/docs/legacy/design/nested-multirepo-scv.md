...
새 구현 엔진은 만들지 않는다. `action:generate` 같은 신규 명령 없음 — **기존 `$scv:codegen` 재사용**. 신규 표면은 선언 명령(`$scv:handoff`)과 additive 동작뿐.
...
`scv/SCV.md` 에 **`SCV:WORKSPACE` 마커 블록** (PROJECT:LOCAL 과 동일 plumbing — `merge.sh` replace_marker_block, `$scv:sync` 가 보존):
...
| `$scv:work` | archive 시점 opt-in 프롬프트: PLAN에 `handoffs:` 있으면 "루트로 전파?"(host user-confirmation mechanism). 비-SINGLE만 발동 | gate 미발동 → 동일 |
