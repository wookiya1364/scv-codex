...
#   graphify (Codex skill — see https://github.com/safishamsi/graphify)
#
...
mode_check_or_install() {
  ...
  if [[ "$pm" == missing-* || "$pm" == "unknown-pm" ]]; then
    ...
    echo ""
    echo "Re-run after installing a package manager. graphify (Codex skill)"
  ...
  echo ""
  echo "graphify (Codex skill, optional):"
if [[ -f "$HOME/.agents/skills/graphify/SKILL.md" ]] || \
     compgen -G "$HOME/.codex/plugins/cache/*/*/skills/graphify/SKILL.md" >/dev/null 2>&1 || \
...
mode_print() {
  ...
  echo "graphify (Codex skill, all OSes):"
  echo "  See https://github.com/safishamsi/graphify"
  echo "  Manual placement: copy SKILL.md to \$HOME/.agents/skills/graphify/SKILL.md"
}
