...
# This script is read-only; it only prints context for Codex to work with.
set -uo pipefail
...
# Extract STANDARD:VERSION value from scv/CODEX.md if present
# (marker name kept as STANDARD:* for backward compatibility; internal-only)
if [[ -f "$SCV_DIR/CODEX.md" ]]; then
  ver=$(awk '
    {
...
for candidate in \
  ...
  "$HOME/.codex/plugins/cache/"*/*/skills/graphify/SKILL.md \
  "$HOME/.codex/plugins/cache/"*/*/*/skills/graphify/SKILL.md; do
  if [[ -f "$candidate" ]]; then
...
# Graph-only mode: stop here after emitting metadata. Codex then decides
...
# The actual split count is content-driven — Codex judges each topic cluster
