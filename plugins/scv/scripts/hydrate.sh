...
if [[ -e "$TARGET/scv" && $FORCE -eq 0 ]]; then
  ...
  echo "  (SCV is non-destructive: it does NOT touch the root CODEX.md — only scv/.)" >&2
  exit 1
fi
...
# Stamp scv/CODEX.md with current SCV version + sync date
# (Root AGENTS.md is user-owned; SCV never touches it.)
VERSION=$(cat "$STANDARD_ROOT/VERSION" | tr -d '[:space:]')
TODAY=$(date +%Y-%m-%d)

if [[ -f "$TARGET/scv/CODEX.md" ]]; then
  replace_simple_marker "$TARGET/scv/CODEX.md" \
    "<!-- STANDARD:VERSION -->" "<!-- /STANDARD:VERSION -->" "$VERSION"
  replace_simple_marker "$TARGET/scv/CODEX.md" \
    "<!-- STANDARD:SYNCED_AT -->" "<!-- /STANDARD:SYNCED_AT -->" "$TODAY"
  echo "  scv/CODEX.md stamped: version=$VERSION synced_at=$TODAY"
fi
...
▶ Next: in your Codex session, run this one line:
