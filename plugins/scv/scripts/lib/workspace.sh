...
#   CHILD  — scv/CODEX.md's SCV:WORKSPACE block declares a non-empty root:.
#
...
SCV_DIR="${SCV_DIR:-scv}"
WS_CODEX="${WS_CODEX:-$SCV_DIR/CODEX.md}"
WS_MANIFEST="${WS_MANIFEST:-$SCV_DIR/WORKSPACE.yaml}"

_ws_block() {
  # Print the SCV:WORKSPACE block body (between markers) of the local scv/CODEX.md.
  [[ -f "$WS_CODEX" ]] || return 0
  extract_marker_block "$WS_CODEX" "SCV:WORKSPACE START" "SCV:WORKSPACE END"
}
