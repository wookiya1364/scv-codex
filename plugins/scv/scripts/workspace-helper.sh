#!/usr/bin/env bash
# workspace-helper.sh — friendly front-door mechanics for $scv:workspace.
#
...
cmd_info() {
  if [[ ! -f "$WS_CODEX" ]]; then
    ...
    echo "HINT: run \$scv:help first to hydrate this project"
    return 0
  fi
...
cmd_init_root() {
  [[ -f "$WS_CODEX" ]] || die "not a hydrated SCV project (run \$scv:help first)"
...
cmd_detach() {
  [[ -f "$WS_CODEX" ]] || die "no scv/CODEX.md here"
  if has_marker_block "$WS_CODEX" "SCV:WORKSPACE START"; then
    replace_marker_block "$WS_CODEX" "SCV:WORKSPACE START" "SCV:WORKSPACE END" '```yaml
repo_id:
role:
root:
workspace:
```'
    ...
    echo "  Re-join anytime with \$scv:workspace (no migration, nothing lost)."
  else
