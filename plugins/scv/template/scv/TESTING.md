...
> Codex follows `INTAKE.md` Step 5 plus the `How to elicit` section below to fill this through user dialogue.
...
- Just before archive: `$scv:work` Step 9a fires user confirmation "run archived regression too?" → on Yes, `$scv:regression` runs as pre-flight; slugs declared in `supersedes` are auto-skipped.
    ...
    steps:
      ...
      - name: Run SCV accumulated regression
        env:
          ...
          SCV_PLUGIN_ROOT: ${{ github.workspace }}/plugins/scv
        run: |
          bash "$SCV_PLUGIN_ROOT/scripts/regression.sh"
...
3. **Runtime triage** — On `$scv:regression` failure, fire a per-slug 3-way user confirmation: regression (fix code) / obsolete (mark now) / flaky (retry).
