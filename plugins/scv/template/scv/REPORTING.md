...
> This document is closer to **configuration**. Codex confirms the user's collab tool and channels in `INTAKE.md` Step 6, then records them in the `PROJECT:LOCAL` block.
...
- Ralph Loop must NOT call HTTP directly. Always go through the `$scv:report <phase> <status>` skill.
