# scv/CODEX.md — SCV workflow index

> **This file is the index and rules for the SCV workflow.** The project root's
> `AGENTS.md` is **never touched by SCV** and remains user-owned. Everything SCV
> needs is under `scv/`.
...
## Relationship with the root AGENTS.md

- The project root's `AGENTS.md` (if any) is the user's **project-wide rules**.
  SCV **never modifies it**.
- SCV's skills, sync, and hydrate routines reference **only this
  `scv/CODEX.md` and other docs under `scv/`**.
- To make Codex aware of SCV in casual conversations too, optionally add this
  one line to your root `AGENTS.md`:
  ```
  > This project uses SCV — see `scv/CODEX.md` for workflow details.
  ```
...
```
project-root/
├── AGENTS.md                    # User-owned (SCV doesn't touch) — optional
├── scv/                          # All SCV workflow docs and state live here
│   ├── CODEX.md                 # this file (SCV index)
...
<!-- Project-wide rules belong in the root AGENTS.md — not this file. -->
<!-- PROJECT:LOCAL END -->
