...
```
Drop into scv/raw/ → $scv:promote (Codex refines via dialogue)
...
This enters **conversation mode** — Codex refines your idea with you (asking goal / scope / acceptance questions), persists every turn to `scv/.conversations/<timestamp>-<slug>.md` (gitignored, local), and offers to draft `PLAN.md + TESTS.md` when there's enough information. You can:
...
| `epic` | — | When splitting a large user request into multiple features, group them under the same epic slug (count is content-driven — Codex proposes + user adjusts). `$scv:status` shows epic progress; `$scv:work`'s PR auto-creation uses the epic branch as base. See §8d |
| `kind` | — | `feature` (default) / `refactor` (epic-closing integration cleanup) / `retirement` (pure removal — §8c). Used by Codex for epic flow / refactor guidance |
| `lang` | — | (v0.7.3+) The resolved language for this promote's content + diagrams + commit/PR text. Set by `$scv:promote` Step 0 — auto-resolved from `settings.json language` and `.env SCV_LANG`, or via user confirmation when those mismatch. Read by `$scv:work` Step 9d and `pr-helper.sh` for full localization (PR title, body labels like `## Summary` / `## 요약` / `## 概要`, footer `🗂 Archived` / `🗂 보관됨` / `🗂 アーカイブ済み`). Values: `english` / `korean` / `japanese` / free-form. Empty / unknown → English fallback. |
...
`$scv:promote` decides this branching automatically. The user only sees the resulting user confirmation when there is a real decision to make (graphify run-or-skip when ARCHITECTURE.md is `N/A`).
...
### When to split (Codex's judgment criteria)
...
If the user explicitly says **"split it"**, ignore the criteria and split. If **"don't split"**, Codex stops proposing splits.
...
| Tests passed + no user direction | Codex asks "archive now?" and waits for answer |
...
| **Auto-propagation** | When A is archived via `$scv:work`, Codex fires user confirmation (default Yes) "mark B as obsolete?" — on approval, modifies B's PLAN.md frontmatter only | Default path triggered when supersedes is declared |
| **Runtime triage** | On `$scv:regression` failure, fire 3-way user confirmation: regression (fix code) / obsolete (mark now) / flaky (retry) | When supersede declaration was missed, or when env changes force deprecation |
...
**Split count is not fixed.** Codex proposes an appropriate count (e.g., 2, 4, 8) and candidate slugs based on the actual content / topic diversity of the raw, and the user does the final adjustment. The §8e example below is split into 7, but **that's just one example, not a recommended standard**.
...
## How to run
\`\`\`bash
bash "$SCV_PLUGIN_ROOT/scripts/regression.sh" --tag payment
