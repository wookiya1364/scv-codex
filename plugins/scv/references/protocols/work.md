...
| Neither (default) | Codex judges. If `Approach Overview` > ~50 lines, `Steps` > ~15, or implementation reveals a dense sub-topic (ARCH / REQUIREMENTS / API / MIGRATION / tests) — **propose** split by asking the user. User accepts or declines. |
...
```
bash ${SCV_PLUGIN_ROOT}/scripts/regression.sh [<SCV_DIR>] --quiet
```
...
| User posture | Action |
|---|---|
| Pre-declared (e.g., "auto-archive when tests pass" / "tests 통과하면 알아서 archive 해", spoken earlier in this conversation) | Auto-invoke: `bash ${SCV_PLUGIN_ROOT}/scripts/work.sh [<SCV_DIR>] <slug> --archive --reason="tests passed"`. Report the ARCHIVED: line. |
...
```
!${SCV_PLUGIN_ROOT}/scripts/deck.sh "scv/archive/<slug>" --lang "<PLAN_LANG>"
```
...
```
bash ${SCV_PLUGIN_ROOT}/scripts/pr-helper.sh [<SCV_DIR>] <slug>
```
