...
Identical to `$scv:work`: resolve from project `.env` `SCV_LANG` →
...
```!
bash "${SCV_PLUGIN_ROOT}/scripts/work.sh" "${SCV_ARGS[@]}"
```
...
```bash
bash "${SCV_PLUGIN_ROOT}/scripts/tests-smell.sh" "$TESTS_FILE"
```
...
After Step 8, the work cycle (report → regression pre-flight → archive decision → supersede propagation → PR creation → epic refactor notice) is identical to `$scv:work` Steps 8–9e. Follow `references/protocols/work.md` verbatim from that point.
...
- Re-implement `$scv:work` Steps 1–5b or 9+ in this file — always link to `references/protocols/work.md`.
