...
You — Codex — drive the accumulated regression: **run every archived TESTS that hasn't been superseded or marked obsolete, then triage any failures with the user**.
...
```!
bash "${SCV_PLUGIN_ROOT}/scripts/regression.sh" "${SCV_ARGS[@]}"
```
...
2. Ask once (optional): "Notify the team about this regression result?"
   - Yes → `bash ${SCV_PLUGIN_ROOT}/scripts/report.sh "accumulated-regression" passed --event regression-summary --summary "<n> slugs passed, <m> skipped (superseded/obsolete)"`
   - No → terminate.
...
[3] "flaky — environmental issue. Let me retry"
    ...
    "Test itself is unstable, or external dependencies (network, timezone, shared resources)
     caused the failure. Codex reruns this slug only, up to 2 times:
     bash ${SCV_PLUGIN_ROOT}/scripts/regression.sh --only <slug>
...
- **[3] flaky**: Run `bash ${SCV_PLUGIN_ROOT}/scripts/regression.sh --only <slug> --quiet` up to 2 times. On pass, log `[flaky→pass on retry N] <slug>`. If both retries fail, re-fire the 3-way dialog.
