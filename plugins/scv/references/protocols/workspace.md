...
```!
bash "${SCV_PLUGIN_ROOT}/scripts/workspace-helper.sh" info
```

Parse `MODE:` → one of `NOT_HYDRATED | SINGLE | ROOT | CHILD`.
- `NOT_HYDRATED` → tell the user to run `$scv:help` first to hydrate, then stop.
...
2. Run:
   ```!
   "${SCV_PLUGIN_ROOT}/scripts/workspace-helper.sh" join --root "<URL>" --id "<id>" --role "<role>" --workspace "<ws>"
   ```
...
1. Run:
   ```!
   "${SCV_PLUGIN_ROOT}/scripts/workspace-helper.sh" init-root
   ```
...
- **분리(detach) — 단일로 되돌림** → run:
  ```!
  "${SCV_PLUGIN_ROOT}/scripts/workspace-helper.sh" detach
  ```
