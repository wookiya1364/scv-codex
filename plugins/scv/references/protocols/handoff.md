...
This is an explicit, human/Codex-judged declaration — SCV never infers cross-repo
...
3. **Write + commit to the root** (commits locally; does NOT push):
   ...
   ```!
   "${SCV_PLUGIN_ROOT}/scripts/handoff.sh" write --to "<to_repo>" --slug "<slug>" --title "<title>" --decision "<needed|maybe|not-needed>" [--from-slug "<slug>"] [--ref-pr "<url>"] --body-file /tmp/scv-handoff-body.md --why-file /tmp/scv-handoff-why.md
   ```
...
4. **Push — ask first, every time.** Show the user one line:
   *"Push this handoff to the workspace root (`<root>`)? It becomes visible to the
   ...
   ```!
   "${SCV_PLUGIN_ROOT}/scripts/handoff.sh" push
   ```
...
```!
bash "${SCV_PLUGIN_ROOT}/scripts/handoff.sh" "<module>" write --to "<to_repo>" --slug "<slug>" --title "<title>" …
```
