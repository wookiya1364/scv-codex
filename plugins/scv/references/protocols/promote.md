...
```!
bash "${SCV_PLUGIN_ROOT}/scripts/handoff.sh" adopt "<handoff_id>"
```
...
```!
bash "${SCV_PLUGIN_ROOT}/scripts/handoff.sh" mark "<handoff_id>" claimed
```
...
```!
bash "${SCV_PLUGIN_ROOT}/scripts/promote-helper.sh" "${SCV_ARGS[@]}"
```
...
| `missing` | anything | Print a **short one-line warning**: "graphify skill not found — proceeding without token-efficient graph queries. Install guide: https://github.com/safishamsi/graphify (place SKILL.md at ~/.agents/skills/graphify/)". Continue. |
...
| Earlier user messages / prior `$scv:<name>` invocations in this conversation | ⚠️ See below |
...
```
!${SCV_PLUGIN_ROOT}/scripts/deck.sh "scv/promote/<folder>" --lang "<LANG_RESOLVED>"
```
...
```
!${SCV_PLUGIN_ROOT}/scripts/readpath.sh update
```
