...
```!
bash "${SCV_PLUGIN_ROOT}/scripts/install-deps.sh" --check
```
...
[1] "Install now"
    ...
    "Run `install-deps.sh --install` to actually install the missing system tools.
     ...
     - graphify (Codex skill) is NOT installed by this command — it has a
       different distribution channel. See https://github.com/safishamsi/graphify
       and place SKILL.md at ~/.agents/skills/graphify/."
...
- **[1] Install now**:
  ```!
  "${SCV_PLUGIN_ROOT}/scripts/install-deps.sh" --install
  ```
...
- **graphify is NOT auto-installed.** It is a Codex skill (not a system CLI) with a different distribution channel. The script's output and Step 1's description always link to https://github.com/safishamsi/graphify so the user can install it manually.
...
- Modify the user's `~/.agents/skills/` directory or attempt to download graphify automatically.
