...
Join the parsed words into one free-form help argument. The shell array keeps
special characters inert; do not rebuild it with `eval`.

```!
bash "${SCV_PLUGIN_ROOT}/scripts/help.sh" "${SCV_ARGS[*]}"
```
...
On choice [1]: run `bash "$SCV_PLUGIN_ROOT/scripts/hydrate.sh" init .`. On [2]:
append `--new`. After hydrate completes, re-run
`bash "$SCV_PLUGIN_ROOT/scripts/help.sh"` and present the new diagnosis. On [3]:
...
- Frontmatter:
  ...
  ---
  ```
- First turn: copy the user's `ARG_CONVERSATION` as the opening user message. Append Codex's response.
