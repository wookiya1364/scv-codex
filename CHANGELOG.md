...
- `core/scripts/metrics.sh` — 기존 기록(아카이브 색인 · 계획서 · 대화 · 결정 로그)만 읽어 지표 넷(계획당 턴 수 · 승인→보관 리드타임 · 후속 재발률 · 순수 절 보유율)을 적용 범위 n/m 과 함께 결정적으로 찍는다. 읽기 전용, `--tsv` 원자료.
- 각 프로젝트의 계기판이지 제품 통계가 아니다 — 원격 텔레메트리·새 로깅 훅 없음.
- Core pin 0.57.0 → 0.58.1 (0.58.1 은 벤더 사본에서 붉던 코어 검사 하나를 고친 패치).

## [0.57.0-codex.1] — 2026-09-21

### Core 0.57.0 동기화 — 후속 다섯

- 가드 예외는 줄 번호가 아니라 문구 앵커 — 문서가 짧아져도 어긋나지 않고, 문구 소실·중복이면 붉음.
- 경계 요구(범위 밖 파일 · 보관 TESTS 본문 · ARCHIVED_AT)는 `core/contracts/boundaries.md` 한 곳, 프로토콜은 한 줄 포인터. 검사 (b) 기준선 0.
- PR 도우미 증거 재실행이 시간 초과(제한 초 + 설정 키)인지 실패(종료 코드)인지 말한다.
- 릴리스 노트의 "올리는 법" 은 실제 명령(`marketplace upgrade` → `plugin add`)으로.
- Core pin 0.56.0 → 0.57.0.

## [0.56.0-codex.1] — 2026-09-21

### Core 0.56.0 동기화 — 미결 6건 후속

- 결정 로그 규칙은 `core/contracts/decisions.md` 한 곳. promote·work·regression 은 스크립트 블록 + 자기 엔트리 블록 + 포인터만.
- 자동 갱신이 파일을 건너뛸 때(PARTIAL·drift) "커밋(또는 되돌리기)하면 다음 액션이 갱신" 안내. 거부는 그대로 거부.
- 규칙 헌법 검사 (b) 에 이유 있는 허용목록 — 의도된 반복 6건은 빠지고 기준선은 11→3. 벤더링 사본은 (b) 를 건너뛰므로 영향 없음.
- Graft 언어 목록 README 일치 확인(23개), no-graph 안내는 이미 있음, 대화 액션 7개 유지.
- Core pin 0.55.0 → 0.56.0.

## [0.55.0-codex.1] — 2026-09-21

### Core 0.55.0 동기화 — Graft 안내 + 규칙 충돌 후속

- Graft 가 없고 지원 언어 파일이 있으면 계획·구현 헤더에 안내 한 줄(효과 + 설치 명령). 셸만 있으면 침묵.
- help: 주제가 명백히 다른 새 요청은 묻지 않고 새 대화. 상시 문구에 "단계 규칙이 먼저(해소 순서 참조)".
- 기록 의무는 `core/contracts/recording.md` 한 곳, 프로토콜은 `## Recording` 한 줄. sync 동의 기준 한 문장.
- 실행기·pr-helper 증거 재실행이 같은 함수로 설정값 누출 차단.
- Core pin 0.54.1 → 0.55.0. 이 래퍼 이슈 #206(업데이트 스킬 문구 참조형)은 별도.

## [0.54.1-codex.1] — 2026-09-20

### Core 0.54.1 동기화 — 규칙 헌법

- `scv/SCV.md` Top-level rules 가 최상위 층: 조항 7개 + 출처 + 해소 순서 한 문단. 우선순위를 말하던
  다른 세 곳은 참조로. 회귀 삭감은 슬러그마다 묻지 않고 결정 표 한 장으로.
- 재발 검사: 우선순위 서술 유일성(게이트) + 중복 요구 래칫(원본 저장소에서만).
- 회귀 실행기가 설정 파일 값을 자식 검사에 흘리지 않음. 보관 색인은 `archive-index.sh` 한 곳에서 재생성.
- 0.54.0 은 건너뜀 — 새 검사가 벤더링 사본의 호스트 표기와 삽입 문장에 걸려 핀 PR 이 만들어지지 않았고,
  0.54.1 이 고쳤다.
- Core pin 0.53.1 → 0.54.1.

## [0.37.0-codex.1] — 2026-08-26

### Core 0.37.0 동기화 — 번호식 화면설계서

- 기획서의 기본형이 "큰 그림 + 번호 마커 + 번호별 상세"로 바뀜. FE 는 상태 띠·
  검증 메시지 표까지, BE 는 호출 화면 표·구성도+순서도·테이블 스키마·실패 응답 표.
- 마커 자동 번호(기본 on), 그림 0장·마커 짝 불일치 경고.
- 순수함수·파이프라인이 모든 계획의 필수 섹션.
...
- README(저장소 가이드 + 플러그인, en/ko/ja)를 백지에서 재구성: "그냥 말 걸면
  됩니다" 우선, 본문 버전 주석 제거, 설정·가드·멀티레포 절 압축.
- Core pin 0.35.1 → 0.35.2 (템플릿 SCV.md 재구성). 동작 변화 없음.
...
- 어떤 SCV 명령이든 시작될 때 `scv/scv_settings.json` 이 없으면 만든다 — 모든 키를
  실제 기본값과 함께, 옛 `.env` 값이 있으면 그 값으로, 맨 위 `_doc` 에 키별 설명과
  허용값. 있으면 새 키만 더하고 사용자 값은 그대로. hydrate 도 실제 파일을 심는다.
- 비밀 파일은 git 이 무시한다고 확인될 때만 만든다(무시 줄이 없으면 `.gitignore` 에
  한 줄 더하고 확인). `.env` 알림은 값이 설정 파일과 다를 때만. 래퍼 자체 변경 없음.
- Core pin `0.33.0` → `0.34.1`, 래퍼 버전 3종 `0.33.0-codex.1` → `0.34.1-codex.1`.

## [0.33.0-codex.1] — 2026-08-24

### Core 0.33.0 동기화 — 설정 이사·템플릿 지문·결정 색인·순수함수 계약

- **설정이 `scv/scv_settings.json` 으로 옮겨졌다.** 프로젝트 루트 `.env` 는 더 이상
  읽지 않는다 — 앱 값과 SCV 값이 섞여 오해가 잦았다. 일반 설정 23개는 커밋되는
  파일로, 토큰·채널 ID 13개는 무시되는 `scv/scv_settings.secret.json` 으로 갈린다.
  **기존 프로젝트는 첫 명령에서 안내를 한 번 받고**, `settings-migrate.sh` 를 한 번
  돌리면 된다(원본 `.env` 는 지우지 않는다). 업데이트는 사용자가 정한 값을 바꾸지
  않고 없는 키만 더한다. `settings-set.sh` 는 비밀 키를 무시되는 파일로 자동으로
  보내므로 실수로 커밋할 경로가 없다.
- **템플릿 갱신이 번호가 아니라 내용으로도 판단한다.** 번호를 안 올려도 템플릿이
  실제로 바뀌었으면 전달된다. 갱신이 안 됐을 때 조용히 넘어가지 않고, status 에
  `[template]` 절이 어긋났을 때만 나온다.
- **결정이 스스로 색인된다.** 결정 로그는 세 지점(계획 승인·보관·폐기 판정)에서
  자동으로 쌓이는데, 이제 위치도 함께 남아 `record-read.sh --key` 로 로그 전체를
  안 읽고 하나만 꺼낸다. `recap.sh` 는 비운 직후 되찾는 자리 — 아무것도 새로 쓰지
  않고 있는 것을 조립한다.
- **순수부와 효과부 계약**(`contracts/purity.md`)과 그것을 강제하는 검사가 들어왔다.
- PR·보고 첨부가 "마지막 실행 전체"에서 "이번 계획 것만"으로 바뀌었다. 옛 동작은
  설정에서 `SCV_ATTACHMENTS_SCOPE=all`.
- Core pin `0.31.2` → `0.33.0`, 래퍼 버전 3종 `0.31.0-codex.2` → `0.33.0-codex.1`.
  래퍼 자체 코드 변경 없음.

## [0.31.0-codex.2] — 2026-08-21

### Core 0.31.2 동기화 — 기록 훅이 다국어 글자를 온전히 남긴다

- 어시스턴트 답변을 일지에 남기는 훅(on-stop.sh)이 꼬리를 바이트 단위로 자르다
  한글·일본어·이모지 글자를 반으로 끊던 결함 수정 — 캡 뒤 반쪽 시퀀스를 떨어내
  일지가 항상 유효한 UTF-8 로 남는다. 래퍼 자체 변경 없음.
- Core pin `0.31.1` → `0.31.2`, 래퍼 버전 3종 `0.31.0-codex.1` → `0.31.0-codex.2`.

## [0.31.0-codex.1] — 2026-08-21

### Core 0.31.1 동기화 — 쉬운 말 2단계 + 기록 훅 등록
...
- Core pin `0.30.0` → `0.31.1`(0.31.0 은 래퍼 검증 단언 하나가 배치를 잘못 전제해 바로 패치), 래퍼 버전 3종 `0.30.0-codex.1` → `0.31.0-codex.1`.
...
- 0.26.0 승격에서 `promote.yml` 이 두 번 조기 종료했고, 두 번 다 손으로 머지했다.
  경로 필터로 건너뛴 매트릭스 잡은 확장되지 않은 이름 그대로 목록에 오르고, 진짜
  잡보다 먼저 도착한다. 대기 조건이 "체크가 몇 개 존재하는가" 였으므로 개수는 즉시
  1 이 되고, 대기가 끝나고, `gh pr checks --watch` 는 완료된 항목 하나를 보고 성공을
  돌려주고, 필수 체크가 생기기도 전에 머지가 거부된다.
- 세 번째 자작 조건을 만드는 대신 머지를 결정하는 값(`mergeStateStatus`)을 본다.
  실패한 체크가 없고, 도는 체크가 없고, GitHub 이 `BLOCKED` 이라 하지 않을 때만
  머지한다. `CLEAN` 하나만 요구하면 아까 그 자리표시자가 rollup 을 계속 `UNSTABLE`
  로 붙잡아 아무것도 머지되지 않는다.

### Added — 벤더 게이트

- `branch-flow.yml` 이 `check-vendor-provenance.sh` 를 돌린다. 봇 브랜치가 아닌
  곳에서 `vendor/scv-core/` 를 다시 쓰는 PR 은 제목에 `[manual-vendor: <이유>]` 가
  없으면 머지 시점에 막힌다 — 기존 `[no-plan: <이유>]` 와 같은 모양이다. 금지가
  아니라 선언인 이유는 Core 계약이 바뀌면 봇이 못 따라오는 경우가 실제로 있어서다.
- 0.25.0 과 0.26.0 모두 릴리스 브랜치에서 Core 를 손으로 심었고, 봇이 연 동기화 PR 은
  도착하자마자 "이미 반영됨" 이 되어 닫혔다. 문제는 중복이 아니라 두 경로가 같은
  일을 하지 않는다는 것이다 — 봇은 게시된 릴리스 아티팩트를 해석해 canonical 과
  materialized 해시를 둘 다 기록하지만, 손으로 복사하면 그 시점 작업 트리에 있던
  것이 기록된다. 이번 0.27.0 은 봇 PR(#63)을 그대로 머지했다.

### Added — SCV Core 0.27.0 동기화

- `check-provenance.sh` 에 처음으로 테스트가 붙었다
  (`core/tests/test-provenance-gates.sh`). 머지를 막는 게이트인데 그동안 검증이
  없었고, 실패 방향이 양쪽 다 조용하다. 두 게이트를 실제 git 저장소 픽스처로
  검증한다.

### 주의

- 승격 대기 수정은 **다음** 승격부터 듣는다. `workflow_dispatch` 는 기본 브랜치의
  워크플로 파일을 실행하므로, 이 릴리스 자체는 아직 옛 로직으로 승격됐다.

## [0.26.0-codex.1] — 2026-08-14

### Changed — SCV Core 0.26.0 동기화 (데크 재설계)

- `$scv:deck` 출력이 다크 전용이 됐다. 테마 토글은 사라지고, 라이트 값은 인쇄에만
  남는다 — 전에는 흰 종이에 검은 판이 20 쪽 찍혔다.
- 다이어그램이 840px 텍스트 칼럼에 0.483 배로 눌려 16px 글자가 7.7px 로 나오던 것을
  고쳤다. 이제 읽기 폭 전체를 쓰고 900px 아래로는 줄이는 대신 스크롤한다. 잘린
  라벨 25 개 → 0.
- 색이 다이어그램 소스에 붙어 있던 팔레트가 아니라 페이지를 따른다. 디스크의 `.md`
  는 그대로라 GitHub 렌더링은 전과 같다.
- 헤더·섹션 내비·푸터가 본문과 같이 스크롤되지 않는다. 첫 문장 앞 크롬 268px →
  109px.

### Fixed — 초록이었지만 망가져 있던 것

- 다이어그램이 파일에 구워진 적이 없었다. `static-mermaid.mjs` 가 Chrome 149 에서
  한 바이트도 내놓지 못해 모든 데크가 CDN 로더를 실어 보냈고, 오프라인 독자는
  다이어그램을 보지 못했다.
- `font:` 축약형 5 곳이 문법 오류라 선언 전체가 버려졌고, 컨트롤 6 개가 브라우저
  기본 폰트로 떴다.

### Verification

- `core/tests/deck-probe.mjs` 가 빌드된 데크 사본에서 계산된 스타일과 실제 기하를
  읽는다. 위 숫자는 전부 여기서 나왔다 — 문자열 고정 검사는 버려진 선언으로 가득한
  스타일시트를 그대로 통과시켰다.

### 주의

- 기존 `.deck.html` 은 빌드 산출물이라 마이그레이션되지 않는다. 새 모습이 필요하면
  다시 생성할 것.

## [0.25.0-codex.2] — 2026-08-13

### Fixed — 가드가 아무것도 거부하지 않고 있었다

- `hooks.json` 의 훅 명령이 `${CODEX_PLUGIN_ROOT}` 를 참조했다. 이 호스트는 그 변수를
  설정하지 않는다 — 훅 환경에 있는 것은 `PLUGIN_ROOT` 와 별칭 `CLAUDE_PLUGIN_ROOT`
  뿐이다. 경로가 `/vendor/scv-core/core/template/hooks/guard.sh` 로 펼쳐져 파일이
  없었고, 훅이 실패했고, 실패한 훅은 동작을 허용한다. `0.25.0-codex.1` 은 설치돼
  보이지만 한 번도 돌지 않은 가드를 싣고 있었다.
- 릴리스된 태그를 임시 Codex 홈에 설치하고 실제 모델에게 계획 파일을 손으로 쓰게
  해서 찾았다. 모델은 썼다. 훅을 계측해 보니 네 번 발화하고 있었으므로 매처와
  등록은 멀쩡했고, 미설정 변수 하나가 전부였다.
- 둘 중 하나를 고르는 대신 `${PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}` 를 쓴다. 둘 다
  설정돼 있고 어느 쪽이 안정된 이름인지 문서화돼 있지 않다.
- `plugins/scv/tests/test-guard-registration.sh` 가 이것을 고정한다 — 모든 훅 명령이
  호스트가 실제로 주는 변수만으로 펼쳐져 존재하는 파일을 가리켜야 한다. 깨진
  빌드에서 계약 테스트가 전부 통과한 이유는 어느 것도 명령을 펼쳐보지 않아서다.

### Fixed — Core 동기화 워크플로

- 오래된 sync 브랜치를 그대로 두지 않고 현재 develop 위에서 다시 만든다. 트리가
  다르면 force-with-lease, 같으면 전처럼 단락한다. 벤더 페이로드 밖의 파일을 지우게
  되면 제안 대신 실패한다.
- `--force-with-lease` 가 같은 잡이 방금 갱신한 remote-tracking ref 와 비교하고 있어
  항상 성립했다 — 이름만 안전한 평범한 `--force` 였다. fetch 전에 원격 tip 을 읽어
  명시적으로 넘긴다.
- "다른 사람이 이 브랜치를 건드렸다" 판별이 `%ae` 였다. git 은 amend·rebase·
  cherry-pick 에서 원저자를 유지하므로 봇 커밋을 사람이 정리해도 봇으로 읽혔다 —
  정확히 그 경우를 잡으려고 쓴 검사가 그 경우를 놓쳤다. `%ce` 로 바꿨다. 그리고
  실패 대신 경고하고 push 를 건너뛴다. 열린 sync PR 브랜치에서 메인테이너가 일하는
  것은 정상 상태이고, 그걸로 야간 잡이 빨개지면 사람들이 잡을 무시하게 된다.
- 병합 없이 닫힌 PR 은 업그레이드 거절로 취급해 다시 제안하지 않는다. 브랜치가
  남아 있어야 그 판정을 되돌릴 수 있으므로 존재를 함께 확인한다.
- 이 run 이 push 하지 않은 브랜치에는 PR 을 열지 않는다. PR 본문이 체크섬·계약·
  회귀 검증을 보증하는데, 그 검증은 같은 스텝이 방금 버린 로컬 커밋에 대해 돈
  것이었다.

### 주의

- `0.25.0-codex.1` 을 설치했다면 업데이트할 것. 그 버전에는 강제가 없다.

## [0.25.0-codex.1] — 2026-08-12

### Added — 워크스페이스 가드 등록

- Core 0.25.0 의 차단형 `PreToolUse` 가드를 플러그인 루트 기본 경로
  (`plugins/scv/hooks/hooks.json`) 에 등록했다. `.codex-plugin/plugin.json` 은 여전히
  `hooks` 키를 선언하지 않는다 — 그 경로는 문서로만 주장돼 있었는데 codex-cli
  0.146.1 에서 실증했다.
- `references/journal-hooks.md` 는 Codex 플러그인 표면이 스스로 훅을 등록할 수 없고
  차단도 불가능하다고 적고 있었다. 둘 다 틀렸다. `PreToolUse` 훅이 `deny` 를
  돌려주면 `apply_patch` 는 파일을 만들기 전에 멈춘다. Codex 가 패치를 `file_path`
  없는 명령 문자열 하나로 넘기므로 가드가 패치 본문에서 대상을 읽어야 한다는 것도
  같이 적었다.

### Added — 프로버넌스 게이트

- `branch-flow.yml` 이 `check-provenance.sh` 를 돌린다. 코드를 바꾸는 PR 은 그 코드가
  나온 계획을 함께 실어야 한다. 릴리스 체인 PR, `chore/core-*` 봇 PR, 문서만 바꾼
  PR 은 면제다. 진짜 예외는 제목의 `[no-plan: <이유>]` — 대괄호가 비어 있으면
  예외로 인정하지 않는다.

### Fixed — 승격 대기

- PR 을 만든 직후에는 체크가 아직 없다. 한 번 묻고 "없음" 으로 결론내던 대기가 그
  창을 밟아, 체크가 곧 통과할 PR 에서 바로 머지로 갔다가 필수 체크 룰에 거부됐다.
  체크가 나타날 때까지 기다리게 했다. 처음 쓴 존재 확인은 `gh pr checks` 의 종료
  코드였는데, 하나라도 대기 중이면 0 이 아니라 대기 내내 "아직" 으로 읽힌다 —
  타임아웃만 길어진 같은 버그다. `statusCheckRollup` 의 개수를 세도록 바꿨다.
  (이 조건은 `0.27.0-codex.1` 에서 다시 고친다.)

### Changed — 이 저장소는 플러그인을 만드는 곳이지 쓰는 곳이 아니다

- `.gitignore` 에 `/scv/` 를 넣었다. 마켓플레이스가 배포하는 페이로드는 추적되는
  트리 전체라, 여기에 `scv/` 가 한 번이라도 생기면 계획서·대화·훅이 받아쓴 저널이
  모든 사용자의 플러그인 페이로드에 실린다. 지금까지 그걸 막는 건 "여기는
  hydrate 하지 않는다" 는 산문 규칙뿐이었다.
- 스킬 4 종(promote, work, codegen, help)에 "직접 흉내내지 말라" 를 넣었다. 15 개
  전부 "언제 쓰라" 는 있었지만 "대신 직접 하지 말라" 는 0 개였다. 모델이 스킬을 안
  부르고 결과물만 만들면 겉보기엔 같지만 raw 라이프사이클도 출처 기록도 남지 않는다.
- `docs/RELEASING.md` 를 만들었다. 릴리스 절차가 어디에도 없었다 — 버전, PR 하나,
  `promote.yml` 세 명령이 전부다. README 3 종의 브랜치 흐름 절에서 링크한다.

### Added — SCV Core 0.25.0 / 0.25.1 동기화

- 가드의 구현이 Core 에 있다. Rule A 는 영수증 없이 `scv/promote/<slug>/` 의
  `PLAN.md` / `TESTS.md` / `FEATURE_ARCHITECTURE.md` 를 새로 만드는 것을 거부하고,
  Rule B 는 영수증 없는 `scv/` 밖 쓰기를 거부한다(`*.md`, `.gitignore`,
  `.gitattributes`, `LICENSE`, 호스트 설정은 면제). 이미 있는 계획 파일 수정은 언제나
  허용된다. 영수증은 호스트가 스스로 내는 이벤트로만 발급된다. Core 계약은 15 개
  명령 전부를 발급 대상으로 정의하지만, 이 호스트에는 mint 항목이 없어 실제 발급은
  아래 "주의" 에 적은 셸 호출 관찰로만 일어난다.
- `core/scripts/env-set.sh` — `sed -i` 없이 `.env` 한 줄을 이식성 있게 쓴다.
  `&` `/` `\` 가 든 값도 원문 그대로 보존한다.
- 0.25.1 은 벤더된 트리에서만 나던 가드 테스트 거짓 실패를 고친다. `test-guard.sh`
  가 배포 payload 에 들어가지 않는 Core 저장소의 호스트 중립성 검사기를 부르고 있어,
  파일이 없는 것이 "호스트 누출" 로 보고됐다. 이 저장소의 벤더 회귀 잡이 그것 때문에
  빨갛게 떴다.

### 주의

- **이 버전의 가드는 한 번도 돌지 않았다.** 훅 명령이 이 호스트에 없는
  `${CODEX_PLUGIN_ROOT}` 를 가리켜, 설치돼 보이지만 아무것도 거부하지 않는 상태로
  나갔다. `0.25.0-codex.2` 에서 고쳤으니 이 버전은 건너뛸 것.
- 훅 신뢰는 훅의 내용에 묶인다. `guard.sh` 가 바뀌는 릴리스마다 `/hooks` 에서 다시
  승인하기 전까지 강제가 없다. 훅은 핫리로드되지 않으므로 업데이트 후 Codex 를
  재시작할 것.
- 가드가 보장하는 것은 "이 세션에서 SCV 를 썼다" 이지 "이 쓰기가 계획된 작업이다"
  가 아니다. 후자는 프로버넌스 게이트가 머지 시점에 본다. 파일을 하나도 안 건드리는
  대화는 둘 다의 사정권 밖이다.
- 강제 수준은 Claude 래퍼보다 약하다. Codex 에는 스킬 호출 이벤트가 없어서 영수증을
  벤더된 Core 스크립트를 부르는 셸 호출을 관찰해 발급한다 — 모델이 스스로 부를 수
  있다. 우발적 경로를 막을 뿐, 작정한 경로는 막지 못한다.
