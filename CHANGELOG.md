...
## [0.33.0-codex.1] — 2026-08-21

### Core 0.32.0 동기화 — PR·보고 첨부는 이번 슬러그 것만

- PR 과 보고에 붙는 영상·스크린샷이 "테스트 결과 폴더의 마지막 실행 전체"에서
  "이번 계획(슬러그) 것만"으로 바뀌었다(경로에 슬러그 포함). 0건이면 그 계획의
  테스트를 한 번 재실행해 이번 영상을 만들고, 그래도 없으면 남의 영상 대신 한 줄 알림.
  `$scv:report --slug`; 없으면 진행 중 계획 1개 추론, 애매하면 예전처럼 최신 파일+알림.
  `.env` `SCV_ATTACHMENTS_SCOPE=all` 로 옛 동작. 래퍼 자체 변경 없음.
- Core pin `0.31.2` → `0.32.0`, 래퍼 버전 3종 `0.31.0-codex.2` → `0.33.0-codex.1`.
